...
router.get('/me', auth, async (req: Request & { userId?: string }, res: Response) => {
  try {
    const { rows } = await pool.query('SELECT id, email, full_name, created_at FROM users WHERE id = $1', [req.userId]);
...
router.put('/me', auth, async (req: Request & { userId?: string }, res: Response) => {
  try {
    const { email, full_name } = req.body || {};
    if (!email && !full_name) return res.status(400).json({ error: 'nothing to update' });
    ...
    const { rows } = await pool.query(
      `UPDATE users SET ${fields.join(', ')} WHERE id = $1 RETURNING id, email, full_name, created_at`,
      [req.userId, ...values]
    );
    if (rows.length === 0) return res.status(404).json({ error: 'user not found' });
    ...
    if (full_name || email) {
      await pool.query(
        `UPDATE profiles SET 
            ...
            updated_at = NOW()
         WHERE user_id = $1`,
        [req.userId, full_name ?? null, email ?? null]
      );
    }
...
router.delete('/me', auth, async (req: Request & { userId?: string }, res: Response) => {
